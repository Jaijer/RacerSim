...
async function handleCreateRace() {
	...
	let countdown = await runCountdown()

	// TODO - call the async function startRace
	let runningRace = await startRace(store.race_id)
...
function runRace(raceID) {
	...
	let raceUpdater = setInterval(async () => {
		...
		*/
		if (current.status == "in-progress") {
			renderAt("#leaderBoard", raceProgress(current.positions))
		}
		/* 
		...
		*/
			...
			renderAt("#race", resultsView(current.positions))
			resolve(current)
		}
	}, 500)
	}).catch(e=>console.log(e, "In runRace"))
...
async function runCountdown() {
	try {
		...
		return new Promise(resolve => {
			...
			perSecondCount = setInterval(() => {
			// TODO - if the countdown is done, clear the interval, resolve the promise, and return
				if (timer == 0) {
					clearInterval(perSecondCount)
					return resolve()
				} else {
		...
		console.log(error + "In runCountdown");
	}
}
...
function accelerate(id) {
	...
	return fetch(`${SERVER}/api/races/${id}/accelerate`, {method: 'POST'}
	  ).catch(e => console.log(e + "In accelerate"))
}
